...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        self.merge_duplicate_documents(orm)
        ...
        # Adding unique constraint on 'ArtefactRepresentation', fields ['artefact', 'md5sum']
        db.create_unique('mediaman_artefactrepresentation', ['artefact_id', 'md5sum'])

    def merge_duplicate_documents(self, orm):
        dups = orm.Document.objects.values('md5sum').annotate(count=Count('md5sum')).filter(count__gt=1)
        for dup in dups:
            docs = orm.Document.objects.filter(md5sum=dup['md5sum'])
            doc1 = docs[0]
            for doc in docs[1:]:
                doc1.museumobject_set.add(*doc.museumobject_set.all())
                doc1.related_people.add(*doc.related_people.all())
                doc.delete()

    def delete_dup_artefactrepresentations(self, orm):
        dups = orm.ArtefactRepresentation.objects.values('md5sum', 'artefact_id').annotate(count=Count('md5sum')).filter(count__gt=1)
        for dup in dups:
            ars = orm.ArtefactRepresentation.objects.filter(md5sum=dup['md5sum'], artefact_id=dup['artefact_id'])
            for ar in ars[1:]:
                ar.delete()
    ...
    def backwards(self, orm):
        ...
        # Removing unique constraint on 'ArtefactRepresentation', fields ['artefact', 'md5sum']
        db.delete_unique('mediaman_artefactrepresentation', ['artefact_id', 'md5sum'])

        # Removing unique constraint on 'Document', fields ['md5sum']
        db.delete_unique('mediaman_document', ['md5sum'])
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 5, 30, 10, 29, 51, 54493)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 5, 30, 10, 29, 51, 54414)'}),
