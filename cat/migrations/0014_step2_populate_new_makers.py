...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm['cat.MuseumObject'].objects.all():
            old_maker = item.old_maker
            if old_maker is not None:
                new_maker, created = orm['parties.Person'].objects.get_or_create(
                    name=old_maker.name,
                    defaults={'display_name': old_maker.name, 'comments': old_maker.comment})
                item.maker = new_maker
                item.save()
    ...
    models = {
        ...
        'cat.museumobject': {
            ...
            'maker': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'made_items'", 'null': 'True', 'to': "orm['parties.Person']"}),
