...
def add_to_collection(modeladmin, request, queryset): #This is an admin/
    """
    Provide an action to add selected objects to existing collection
    """
    selected = request.POST.getlist(admin.ACTION_CHECKBOX_NAME)
    return HttpResponseRedirect("/collection/add_objects?ids=%s" % ",".join(selected))
...
# This is passed in for specific merge links. This id comes from the linking model (Person, ...)
